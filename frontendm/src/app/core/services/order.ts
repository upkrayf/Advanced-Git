...
export class OrderService {
  ...
  getOrders(params?: {
    ...
    return this.http.get<OrderModel[]>(this.apiUrl, { params: p });
  }
  ...
  getMyOrders(page = 0, size = 20): Observable<OrderModel[]> {
    const p = new HttpParams().set('page', page).set('size', size);
    return this.http.get<OrderModel[]>(`${this.apiUrl}/my`, { params: p });
  }
  ...
  checkout(checkoutData: any): Observable<OrderModel> {
    return this.http.post<OrderModel>(`${this.apiUrl}/checkout`, checkoutData);
  }
